...
// Cards were bumped up in size (see .fan-card in globals.css); this scale
// keeps the fan spread and height offsets proportional to the bigger cards
// instead of leaving them clustered at the old spacing.
const SIZE_SCALE = 1.15;
const HOVER_Z = 100;

const FAN_POSITIONS = [
  { rot: -21, scale: 0.7756, x: -30 * SIZE_SCALE, y: 7.3 * SIZE_SCALE, zIndex: 1 },
  { rot: -14, scale: 0.8498, x: -22 * SIZE_SCALE, y: 4.0 * SIZE_SCALE, zIndex: 2 },
  { rot: -7,  scale: 0.9346, x: -11 * SIZE_SCALE, y: 1.3 * SIZE_SCALE, zIndex: 3 },
...
function getHeightMultiplier(width: number) {
  let idealPx: number;
  if (width < 480) idealPx = 22 * 16 * SIZE_SCALE;
  else if (width < 640) idealPx = 26 * 16 * SIZE_SCALE;
  else if (width < 768) idealPx = 28 * 16 * SIZE_SCALE;
  else if (width < 1024) idealPx = 34 * 16 * SIZE_SCALE;
  else idealPx = 38 * 16 * SIZE_SCALE;
...
function getSlotConfig(totalCards: number, slot: number) {
  ...
  return {
    ...
    x: distance * 30 * SIZE_SCALE,
    y: absDistance * absDistance * 7.3 * SIZE_SCALE,
  ...
  useEffect(() => {
    ...
    cardElements.forEach((card, cardIndex) => {
      ...
      if (slot !== undefined) {
        ...
        if (isFirstMount) {
          // Covers should be visible the instant the page renders, not after
          // a staggered bounce-in — set directly to the resting position.
    ...
    const updateHoverLayout = (hoveredSlot: number | null) => {
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        gsap.set(el, { zIndex: slot === hoveredSlot ? HOVER_Z : base.zIndex });
      });
    };
